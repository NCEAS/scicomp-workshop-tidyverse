...
Let's re-create our year x body mass plot but let's make it a scatter plot by adding `geom_point`.
...
Because `aes` gets all of the variable information, we don't need to give *anything* to `geom_point`! This makes adding multiple geometries much easier than if we had to re-specify the variables separately in every geometry!
:::
...
Great! We could of course just add the `ggplot` and `geom_point` and `geom_smooth` together at the same time but the `+` allows us to build graphics step-by-step if we so desire.
:::
...
Let's make a plot of the number of penguins per island per species with more informative axis labels and a plot title. We'll need to calculate that quickly but we can leverage the tools we practices in the "Summarize" chapter to do that.
...
1.  If you want to change the color(s) used by the `color` argument of `aes` you must use `scale_color_manual`. `ggplot2` defines "color" as the outlines of shapes or the whole shape **_if_** the shape isn't hollow and defines "fill" as the interior of hollow shapes. If you ever try to manually specify one or the other and your plot looks the same, chances are you're using the wrong `scale_..._manual` function!

2. You can use "hexadecimal codes" for colors (sort of a machine-readable way of specifying colors) in this function. We find the website [colorbrewer2.org](www.colorbrewer2.org) particularly useful because it groups colors into palettes and includes a "colorblind safe" check box that limits your options to only those that will be accessible to all viewers.
:::
...
If that all seemed pretty involved, don't worry! `ggplot2` comes with several `theme_...` functions that are pre-built and modify a lot of these parameters in desirable ways without necessitating you doing a deep dive into Google. For instance, `theme_bw` or `theme_classic` or both great options that we'll demonstrate below.

:::callout-note
## Example

Let's re-use the `core_plot` object we build above to quickly demonstrate `theme_bw` and `theme_classic`. You've likely seen the default `ggplot2` theme *ad nauseam* at this point but let's show it one more time for comparative purposes.
...
Now that is looking publication-quality! One quick note though: **whichever theme specification is last "wins".** This means that if you set font size to 14 and then add a `theme` argument beneath it that sets it to 30, the font size of your plot will be 30. This is part of why you can add pre-built themes and your own custom modifications together but is good to keep in mind if you're sure your custom theme is specified correctly but isn't showing up in your plot.
:::
...
my_theme <- theme_classic() +
  # As before, we're moving the plot and increasing axis font size
