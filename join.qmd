...
penguins_left_joined <- penguins_tidy %>%
  ...
  dplyr::left_join(y = penguins_raw, by = "record_number")

dplyr::glimpse(penguins_left_joined)
```

What we have in the end is `penguins_left_joined`, a dataframe with information from both `penguins_tidy` and `penguins_raw`! *All* of the rows in our `filter`ed `penguins_tidy` are kept but only the rows from `penguins_raw` that have a matching `record_number` in `penguins_tidy` are included.
:::
