...
- <u>Use</u> `dplyr`'s `select` function to select relevant columns from your dataframe
- <u>Use</u> `dplyr`'s `filter` function along with logical and boolean operators to subset rows
- <u>Use</u> `dplyr`'s `mutate` and `case_when` functions to create and modify columns 
- <u>Use</u> `tidyr`'s `separate` function to split a column into two
